...
function MapCircleController({
  ...
  useEffect(() => {
    if (center) {
      ...
      // Only fly when the new pin is outside the current view. Clicks are by
      // definition inside the view, so the map stays put — no drift across
      // pin drops. Search results that land far away still get a fly.
      const pinInView = map.getBounds().contains([center.lat, center.lng]);
      if (!pinInView) {
...
interface MapClientDarkProps {
  ...
  radiusKm?: number;
  /**
   * Pixels at the bottom of the map covered by an overlay (e.g. a mobile
   * bottom sheet). Used as extra fitBounds padding so the pin lands in the
   * visible region.
   */
  bottomInset?: number;
}
...
export default function MapClientDark({
  ...
  // The visible tip of the rotated 20×20 square sits at (10, 27) of a 20×20
  // box because the rotated diagonal + the 2px border push the corner ~7px
  // past the bottom edge. iconAnchor is set to that tip so the geographic
  // point matches the visual tip exactly. The HTML wrapper is sized to
  // include the overhang so leaflet's bounding box matches what's drawn.
  const pinIcon = useMemo(() => {
    if (typeof window === "undefined") return undefined;
    return L.divIcon({
      className: "",
      html: `
        <div style="position:relative;width:20px;height:27px;">
          <div class="valyou-pin-drop">
            <div style="width:100%;height:100%;background:#C3110F;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:2px solid white;box-shadow:0 2px 10px rgba(195,17,15,0.6);"></div>
          </div>
          <div class="valyou-pin-ripple" style="bottom:0"></div>
        </div>`,
      iconSize: [20, 27],
      iconAnchor: [10, 27],
    });
