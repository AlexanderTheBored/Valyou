...
export default function NavbarDark() {
  ...
  return (
    <nav className="bg-surface/95 supports-[backdrop-filter]:bg-surface/80 backdrop-blur-md border-b border-black/[0.07] dark:border-white/[0.07] h-[60px] flex items-center px-4 sm:px-6 justify-between sticky top-0 z-50">
      ...
      <Link
        ...
        className={`hidden md:flex absolute left-1/2 -translate-x-1/2 h-full items-center px-4 text-sm border-b-2 transition-colors ${pathname === "/map"
            ? "border-[#C3110F] text-ink font-semibold"
      ...
      <div className="hidden md:flex items-center gap-1 text-sm shrink-0">
        <ThemeToggle className="text-ink/50 hover:text-ink hover:bg-black/5 dark:hover:bg-white/10 mr-1" />

        {!loggedIn ? (
          <>
            <Link href="/auth" className="text-ink/60 hover:text-ink transition-colors px-3 py-1.5 rounded-md">
              Log In
            </Link>
            <Link
          ...
          user?.username && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-all text-sm font-semibold text-ink"
              >
              ...
              {dropdownOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-1.5 w-48 bg-surface border border-black/10 dark:border-white/10 rounded-xl shadow-xl z-20 overflow-hidden valyou-fade-up">
                    <div className="px-4 py-2.5 border-b border-black/[0.05] dark:border-white/[0.05]">
                      <p className="text-[10px] text-ink/45 font-bold uppercase tracking-wider">Logged in as</p>
                      <p className="text-xs font-bold text-ink truncate mt-0.5">{user.username}</p>
                    </div>
                    <Link
                      href="/valuations"
                      onClick={() => setDropdownOpen(false)}
                      className="block w-full text-left px-4 py-2.5 text-xs text-ink hover:bg-black/[0.03] dark:hover:bg-white/[0.03] font-semibold transition-colors border-b border-black/[0.05] dark:border-white/[0.05]"
                    >
                      Previous Valuations
                    </Link>
                    <button
      ...
      <div className="flex md:hidden items-center gap-1">
        ...
        <button
          ...
          className="w-9 h-9 flex items-center justify-center rounded-md text-ink/70 hover:bg-black/5 dark:hover:bg-white/10 transition-colors"
        >
      ...
      {menuOpen && (
        <>
          <div
            ...
            aria-hidden
          />
          <div className="absolute md:hidden top-full inset-x-0 bg-surface border-b border-black/[0.07] dark:border-white/[0.07] shadow-lg z-50 valyou-fade-up">
            <div className="px-4 py-3 flex flex-col gap-1">
              <Link
                ...
                className={`flex items-center gap-2 px-3 py-3 rounded-lg text-sm transition-colors ${pathname === "/map"
                    ...
                    : "text-ink/70 hover:bg-black/5 dark:hover:bg-white/5"
                  }`}
              >
                <MapPin size={14} />
                Explore Map
              ...
              {!loggedIn ? (
                <div className="flex gap-2 pt-2 mt-1 border-t border-black/[0.06] dark:border-white/[0.06]">
                  <Link
                    href="/auth"
                    className="flex-1 text-center py-2.5 rounded-md text-sm border border-black/10 dark:border-white/10 text-ink/70 hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                  >
                    Log In
                  </Link>
                  <Link
                ...
                user?.username && (
                  <div className="flex flex-col gap-3 py-2 border-t border-black/[0.06] dark:border-white/[0.06]">
                    ...
                    <Link
                      href="/valuations"
                      onClick={() => setMenuOpen(false)}
                      className="w-full text-center py-2.5 rounded-md text-sm bg-black/[0.04] dark:bg-white/[0.04] text-ink font-bold hover:bg-black/[0.08] dark:hover:bg-white/[0.08] transition-colors"
                    >
                      Previous Valuations
                    </Link>
                    <button
