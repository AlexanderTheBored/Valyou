...
const MapClientDark = dynamic(() => import("@/components/MapClientDark"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full bg-canvas flex items-center justify-center min-h-[240px]">
  ...
  return (
    <div className="min-h-screen bg-canvas text-ink flex flex-col">
        ...
        <div>
          <div className="flex items-center gap-2 text-xs text-ink/50 mb-1">
            <button
              ...
              className="hover:text-ink font-medium flex items-center gap-0.5"
            >
          ...
          <div className="flex flex-col gap-1 mt-1">
            ...
            {secondaryLocation && (
              <p className="text-xs text-ink/50 pl-6 font-medium leading-none">
                {secondaryLocation}
              </p>
            )}
      ...
      <div className="max-w-5xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6 sm:space-y-8">
        ...
        {/* Main Pricing Banner */}
        <div className="bg-surface border border-black/[0.06] dark:border-white/[0.06] rounded-2xl shadow-sm relative overflow-hidden">
          {/* Embedded Wide Map */}
          <div className="h-[280px] w-full border-b border-black/[0.06] dark:border-white/[0.06] relative">
            <MapClientDark
              ...
              interactive={false}
            />
            <div className="absolute top-4 left-4 z-10 bg-surface/90 backdrop-blur-md px-3 py-1.5 rounded-xl border border-black/5 dark:border-white/5 text-[10px] font-bold uppercase tracking-wider text-ink/75 shadow-md no-print">
          ...
          <div className="p-6 md:p-8 relative">
            <div className="text-center md:text-left flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div>
                <p className="text-[10px] font-bold text-ink/45 uppercase tracking-[0.2em] mb-2">
                  Estimated Fair Market Value
                </p>
                <h2 className="text-4xl md:text-5xl font-black text-ink tabular-nums tracking-tight leading-none">
              ...
              <div className="flex flex-col items-center md:items-end justify-center shrink-0 border-t md:border-t-0 md:border-l border-black/[0.06] dark:border-white/[0.06] pt-4 md:pt-0 md:pl-8">
                ...
                <p className="text-[10px] text-ink/45 mt-2 font-bold uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Market Value Card */}
          <div className="bg-surface border border-[#C3110F]/20 dark:border-[#C3110F]/30 rounded-xl p-5 shadow-sm relative overflow-hidden">
            ...
            <h3 className="text-xl font-bold text-ink">₱{valuation.total_value.toLocaleString(undefined, { maximumFractionDigits: 0 })}</h3>
            <p className="text-xs text-ink/50 mt-1 font-medium">Computed fair value rate</p>
            <div className="border-t border-black/[0.04] dark:border-white/[0.04] mt-4 pt-3 text-[10px] text-ink/45 leading-relaxed">
              Standard rate: <strong className="text-ink font-mono">₱{Math.round(valuation.market_value).toLocaleString()}/sqm</strong>. Derived using multivariate spatial regression.
            </div>
          </div>

          {/* BIR Zonal Value Card */}
          <div className="bg-surface border border-blue-500/20 dark:border-blue-500/30 rounded-xl p-5 shadow-sm relative">
            ...
            <h3 className="text-xl font-bold text-ink">₱{(valuation.zonal_value * valuation.area).toLocaleString(undefined, { maximumFractionDigits: 0 })}</h3>
            <p className="text-xs text-ink/50 mt-1 font-medium">Official statutory reference</p>
            <div className="border-t border-black/[0.04] dark:border-white/[0.04] mt-4 pt-3 text-[10px] text-ink/45 leading-relaxed">
              Standard rate: <strong className="text-ink font-mono">₱{Math.round(valuation.zonal_value).toLocaleString()}/sqm</strong>. Used as capital gains tax baseline.
            </div>
          </div>

          {/* Scraped Listing Value Card */}
          <div className="bg-surface border border-orange-500/20 dark:border-orange-500/30 rounded-xl p-5 shadow-sm relative">
            ...
            <h3 className="text-xl font-bold text-ink">₱{(valuation.scraped_value * valuation.area).toLocaleString(undefined, { maximumFractionDigits: 0 })}</h3>
            <p className="text-xs text-ink/50 mt-1 font-medium">Active listings baseline</p>
            <div className="border-t border-black/[0.04] dark:border-white/[0.04] mt-4 pt-3 text-[10px] text-ink/45 leading-relaxed">
              Standard rate: <strong className="text-ink font-mono">₱{Math.round(valuation.scraped_value).toLocaleString()}/sqm</strong>. Average price extracted from active comparisons.
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-surface border border-black/[0.06] dark:border-white/[0.06] rounded-xl p-5 sm:p-6 shadow-sm flex flex-col">
            <h3 className="text-xs font-bold text-ink uppercase tracking-[0.15em] mb-4">
              Property Profile
            </h3>
              ...
              {[
                ...
                >
                  <div className="flex items-center gap-2 mb-2.5">
                    ...
                    <span className="text-[10px] font-bold text-ink/55 uppercase tracking-wider leading-tight">
                      {label}
                    </span>
                  </div>
          ...
          <div className="bg-surface border border-black/[0.06] dark:border-white/[0.06] rounded-xl p-5 sm:p-6 shadow-sm flex flex-col relative overflow-hidden">
            <p className="text-[9px] font-bold text-[#C3110F] uppercase tracking-wider mb-1">
              Statistical Analysis
            </p>
            <h3 className="text-[10px] font-bold text-ink/50 uppercase tracking-[0.15em] mb-5">
              Valuation Reliability
            </h3>
            ...
            <div className="flex items-center gap-6 flex-1">
              ...
              <div className="relative w-40 h-40 shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-1">
                  <span className="text-4xl font-black tabular-nums tracking-tight leading-none text-ink">
                    {r2Pct}%
                  </span>
                  <span className="text-[9px] font-semibold uppercase tracking-widest text-ink/30">
                    R²
                  </span>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col justify-center">
                ...
                <p className="text-5xl font-black tabular-nums leading-none tracking-tight text-ink">
                  {valuation.listings_used}
                </p>
                <p className="text-[10px] font-semibold text-ink/30 mt-2 tracking-wide">
                  comparisons analyzed
                </p>
              </div>
            </div>

            <div className="border-t border-black/[0.04] dark:border-white/[0.04] mt-5 pt-3 text-[10px] text-ink/45 leading-relaxed no-print">
              R² scores above <strong className="text-ink">70%</strong> indicate strong alignment with local geographic market trends.
            </div>
          </div>
        </div>

        {/* Recalculate location section */}
        <div className="bg-surface border border-black/[0.06] dark:border-white/[0.06] rounded-xl p-5 sm:p-6 shadow-sm no-print">
          <h3 className="text-xs font-bold text-ink uppercase tracking-[0.15em] mb-2 flex items-center gap-1.5">
            <Activity size={14} className="text-[#C3110F]" />
            Recalculate Zonal Value
          </h3>
          <p className="text-[11px] text-ink/50 mb-4 leading-normal font-medium">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[9px] font-bold text-ink/55 uppercase tracking-wider">Province</label>
                  <select
                    value={selectedProvince}
                    onChange={(e) => onProvinceChange(e.target.value)}
                    className="bg-black/[0.02] dark:bg-white/[0.02] border border-black/10 dark:border-white/10 rounded-xl px-3 py-2 text-xs font-semibold outline-none focus:border-[#C3110F]/40 transition-colors text-ink"
                  >
                    <option value="" disabled className="bg-surface">Select Province...</option>
                    {provinceOptions.map((prov) => (
                      <option key={prov} value={prov} className="bg-surface">{prov}</option>
                    ))}
                  </select>
                </div>
                ...
                {/* City Dropdown */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-[9px] font-bold text-ink/55 uppercase tracking-wider">City / Municipality</label>
                  <select
                    value={selectedCity}
                    onChange={(e) => onCityChange(e.target.value)}
                    disabled={!selectedProvince}
                    className="bg-black/[0.02] dark:bg-white/[0.02] border border-black/10 dark:border-white/10 rounded-xl px-3 py-2 text-xs font-semibold outline-none focus:border-[#C3110F]/40 transition-colors text-ink disabled:opacity-40"
                  >
                ...
                {/* Barangay Dropdown */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-[9px] font-bold text-ink/55 uppercase tracking-wider">Barangay</label>
                  <select
                    ...
                    className="bg-black/[0.02] dark:bg-white/[0.02] border border-black/10 dark:border-white/10 rounded-xl px-3 py-2 text-xs font-semibold outline-none focus:border-[#C3110F]/40 transition-colors text-ink disabled:opacity-40"
                  >
                    <option value="" disabled className="bg-surface">Select Barangay...</option>
                    {barangayOptions.map((brgy) => (
                      <option key={brgy} value={brgy} className="bg-surface">{brgy}</option>
                    ))}
                  </select>
                </div>
                ...
                {/* Street Dropdown */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-[9px] font-bold text-ink/55 uppercase tracking-wider">Street</label>
                  <select
                    ...
                    className="bg-black/[0.02] dark:bg-white/[0.02] border border-black/10 dark:border-white/10 rounded-xl px-3 py-2 text-xs font-semibold outline-none focus:border-[#C3110F]/40 transition-colors text-ink disabled:opacity-40"
                  >
                    <option value="" disabled className="bg-surface">Select Street...</option>
                    {streetOptions.map((str) => (
                      <option key={str} value={str} className="bg-surface">{str}</option>
                    ))}
                  </select>
                </div>
              </div>
        ...
        {pdfUrl && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-surface border border-black/10 dark:border-white/10 rounded-2xl w-full max-w-4xl h-[85vh] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
              {/* Modal Header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-black/5 dark:border-white/5 bg-black/[0.01] dark:bg-white/[0.01]">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-ink/50 hover:text-ink px-3 py-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-all"
                  >
                    Close Preview
                  </button>
                </div>
